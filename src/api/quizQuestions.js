var quizQuestions = [
  {
      question: "The novel coronavirus only seriously affects older people.",
      info:`While statistically speaking older people are more severely affected, that is skewed 
            by the fact that they are a part of the population that tend to have pre-existing 
	    medical conditions. 
	    The fact here is that the COVID-19 affects anyone with pre-existing medical conditions.
	    People of all ages can be infected by the new coronavirus. 
	    Older people, and people with pre-existing medical conditions (such as asthma, diabetes, heart disease)
	    appear to be more vulnerable to becoming severely ill with the virus.
 	    WHO advises people of all ages to take steps to protect themselves from the virus,
	    for example by following good hand hygiene and good respiratory hygiene.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
	  {
	      type: "False",
	      content: "False"
	  }
      ],
      correctAnswer: "False"
  },
  {
      question: "Antibiotics are effective in preventing and treating the new coronavirus.",
      info: `No, antibiotics do not work against viruses. They only work against bacteria.
	    The new coronavirus is well, a virus and therefore antibiotics should 
            not be used as a means of prevention or treatment.
	    However, if you are hospitalized for COVID-19, you may receive antibiotics to 
	    keep you from getting a bacterial co-infection due to your low immunity.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
          {
              type: "False",
              content: "False"
          }
      ],
      correctAnswer: "False"
  },
  {
      question: "There are medicines to treat the new coronavirus.",
      info:  `There is no specific drug to prevent or treat the new 
      	     coronavirus, yet. However, those infected with the virus should receive 
	     appropriate care to relieve and treat symptoms, and those with severe illness 
	     should receive optimized supportive care. Some specific treatments are under 
	     investigation, and will be tested through clinical trials. WHO is helping to 
	     accelerate research and development efforts with a range of partners.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
          {
              type: "False",
              content: "False"
          }
      ],
      correctAnswer: "False"
  },
  {
      question: "The new coronavirus was deliberately created or released by people.",
      info: `Viruses can change over time. Occasionally, a disease outbreak happens when a virus 
      that is common in an animal such as a pig, bat or bird undergoes mutation and passes to humans. 
      This is likely how the new coronavirus came to be.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
          {
              type: "False",
              content: "False"
          }
      ],
      correctAnswer: "False"
  },
  {
      question: "Is coronavirus the deadliest virus known to man?",
      info: `Although SARS-CoV-2 is much more lethal than influenza, it is not 
      	    the deadliest virus that people have faced. Others, such as Ebola, have higher 
	    mortality rates.`,
      answers: [
          {
              type: "True",
              content: "Yes"
          },
          {
              type: "False",
              content: "No"
          }
      ],
      correctAnswer: "False"
  },
  {
      question: "Can eating garlic help prevent being infected by the new coronavirus?",
      info: `Garlic is a healthy food that may have some antimicrobial properties. However, 
      	    there is no evidence from the current outbreak that eating garlic has protected
	    people from the new coronavirus.`,
      answers: [
          {
              type: "True",
              content: "Yes"
          },
          {
              type: "False",
              content: "No"
          }
      ],
      correctAnswer: "False"
  },
  {
      question: "Coronavirus cannot survive in Hot and Humid areas.",
      info: `There is no proper evidence to suggest that Coronavirus can not survive hot and 
      humid weather. It has been a widely circulated rumor about the novel coronavirus, that it cannot 
      withstand summers or high-temperature weathers. As per the WHO international Website – 
      ‘From the evidence so far, the COVID-19 virus can be transmitted in ALL AREAS, including 
      areas with hot and humid weather’.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
          {
              type: "False",
              content: "False"
          }
      ],
      correctAnswer: "False"
  },
  {
      question: "There will be a vaccine available for COVID-19 in the next 1-2 years.",
      info: `Vaccines are under development and will enter human trials soon. However it could take 
      anywhere between 12 to 18 months for a vaccine to be deployable.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
          {
              type: "False",
              content: "False"
          }
      ],
      correctAnswer: "True"
  },
  {
      question: "Wearing protective face coverings is helpful in mitigating the spread of COVID-19.",
      info: `As per the updated guidelines from the CDC wearing masks while out in public spaces can reduce the 
      probability of the infected spreading the virus to the others. It is advisable for everyone to wear face coverings 
      as there's a high chance of people being asymtomatic carriers.`,
      answers: [
          {
              type: "True",
              content: "True"
          },
          {
              type: "False",
              content: "False"
          }
      ],
      correctAnswer: "True"
  },
  {
      question: "Is it safe to receive a letter or a package from China?",
      info: `Yes, it is safe. People receiving packages from China are not at risk of 
      contracting the new coronavirus. From previous analysis, we know coronaviruses do not 
      survive long on objects, such as letters or packages. The maximum they can survive is 48 hours
      on hard surfaces.`,
      answers: [
          {
              type: "True",
              content: "Yes"
          },
          {
              type: "False",
              content: "No"
          }
      ],
      correctAnswer: "True"
  }
];

export default quizQuestions;
